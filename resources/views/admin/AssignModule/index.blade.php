                        ...
                        <div class="card-header">
                            ...
                            <div class="pull-right box-tools">
                                <div class="float-right mt-1">
                                    <a class="btn btn-primary uppercase text-bold" href="{{ route('admin.module.assign.create') }}">New Module Assign Add</a>
                                </div>
                            </div>
                        </div>
                        <!-- /.card-header -->
                            ...
                            <div class="table-responsive">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tr>
                                        ...
                                        <th class="text-bold text-uppercase">Module name</th>
                                        <th class="text-bold text-uppercase">Exam name</th>
                                        <th class="text-bold text-uppercase">Action</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach ($assignModules as $key => $assignModules)
                                        <tr>
                                            <td>{{ ++$key }}</td>
                                            <td>{{ $assignModules->module ? $assignModules->module->moduleName : 'No Module Assigned' }}</td>


                                            <td>{{ $assignModules->exam ? $assignModules->exam->name : 'No Exam Assigned' }}</td>

                                            <td>
                                                @if($assignModules->id !== 1)
                                                 <a class="btn btn-sm btn-primary fa fa-edit" href="{{ route('admin.module.assign.edit',$assignModules->id) }}" title="Edit"></a>
                                                   @can('delete')
                                                       {!! Form::button('<i class="fa fa-trash"></i> ', ['title' => 'Delete', 'class' => 'btn btn-sm btn-danger bold uppercase delete_button','data-toggle'=>"modal",'data-target'=>"#DelModal",'data-id'=>$assignModules->id]) !!}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer justify-content-between">
                    <form action="{{route('admin.module.assign.destroy',0)}}" method="post" id="deleteForm">
                        {!! csrf_field() !!}
                        {!! method_field('DELETE') !!}
    ...
    <script>
        $(document).ready(function () {
            $(document).on("click", '.delete_button', function (e) {
                var id = $(this).data('id');
                var url = '{{ route("admin.module.assign.destroy",":id") }}';
