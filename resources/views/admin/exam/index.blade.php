                        ...
                        <div class="card-header">
                            ...
                            <div class="pull-right box-tools">
                                <div class="float-right mt-1">
                                    <a class="btn btn-primary uppercase text-bold" href="{{ route('admin.subject.create') }}"> New Subject Add</a>
                                </div>
                            </div>
                        </div>
                        <!-- /.card-header -->
                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    @foreach ($batchs as $key => $batch)
                                        <tr>
                                            ...
                                            <td>
                                                @if($batch->id !== 1)
                                                 <a class="btn btn-sm btn-primary fa fa-edit" href="{{ route('admin.batch.edit',$batch->id) }}" title="Edit"></a>
                                                   @can('delete')
                                                       {!! Form::button('<i class="fa fa-trash"></i> ', ['title' => 'Delete', 'class' => 'btn btn-sm btn-danger bold uppercase delete_button','data-toggle'=>"modal",'data-target'=>"#DelModal",'data-id'=>$batch->id]) !!}
                                                   @endcan
                                                @endif
                                            </td>
                                        </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer justify-content-between">
                    <form action="{{route('admin.batch.destroy',0)}}" method="post" id="deleteForm">
                        {!! csrf_field() !!}
                        {!! method_field('DELETE') !!}
    ...
    <script>
        $(document).ready(function () {
            $(document).on("click", '.delete_button', function (e) {
                var id = $(this).data('id');
                var url = '{{ route("admin.batch.destroy",":id") }}';
