                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="Course_name">Module Name<code>*</code></label>
                                    <input type="text" class="form-control" id="moduleName" name="moduleName" value="{{ old('moduleName') }}" required placeholder="module Name">
                                </div>
                            </div>
                        </div>
                        <div class="card-footer">
                            <button type="submit" class="btn btn-primary float-right">Add Module</button>
                        </div>
                    </form>
                </div>
            <!-- /.card -->
...
<script>
    $(document).ready(function() {
        $('#subjectForm').on('submit', function(e) {
            ...
            let moduleName = $('#moduleName').val(); // Get the subject name
            $.ajax({
                url: "{{ route('admin.moduleMcq.store') }}", // The route for storing the subject
                type: "POST"
                , data: {
